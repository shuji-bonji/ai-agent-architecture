...
- 前後ナビゲーションは本文に書かない。VitePress が docFooter として自動で出す（順序はサイドバーに従う）
...
- 前後ナビゲーションを本文に手書きしない。VitePress が docFooter として自動で出すため、書くと同じものが 2 つ並ぶ
  - 自動の順序はサイドバー（`docs/.vitepress/locales/*.ts`）の並びで決まる。変えたいときはサイドバーを直す
  - 特定のページだけ行き先を変えるなら、frontmatter の `prev` / `next` で指定する
- Discussion / 出典リンクがある場合は、本文の末尾に `---` で区切って配置する
