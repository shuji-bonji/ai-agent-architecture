...
MCP は「外部に繋ぐ手段」の 1 つに過ぎない。エージェントが外部に触れるのは常に**[ハーネス](../glossary#harness)（の手先）**であり、MCP・直接 HTTP・A2A・プラグインはどれも「ハーネスのツール連携責務の中身」の分類でしかない（→ [strategy/harness-engineering-mapping](../strategy/harness-engineering-mapping) の大原則）。MCP を設計する前に、**全 I/F の棚の中で MCP がどこに位置するか**を俯瞰しておく。
...
- ❌ **コンテキスト消費**
  ツール定義の読み込みだけで[トークン](../glossary#token)を消費。特にツール数が多い場合、[コンテキストウィンドウ](../glossary#context-window)を圧迫する可能性あり。
