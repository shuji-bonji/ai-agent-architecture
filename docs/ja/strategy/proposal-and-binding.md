...
> **3 行で言うと**
>
> - 拘束するかどうかは **成果物がトークン列に入るか** で決まる。処理そのものが決定論的かどうかとは無関係である。
...
| **トークン列の外にあるもの**   | permission、hook、MCP サーバ側の引数検証、型チェック、CI、テスト          | する                       |
...
| **拘束** | する     | 通すか、拒否するか         | permission / hook、サーバ側検証、型・CI・テスト    |
