...
|                              |                                                                                                                          |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **このページで固定するもの** | 三段分離の定義、判定層の帰属判断（系の外を含む）、四値判定、ルール表の書式、解説層の検査、再現性の担保手順、回帰監視                                     |
...
### 基準が書けても置けないことがある — 系の外

上の表は「基準を事前に書けるか」の 1 問で分岐している。この問いの手前に、もう 1 問ある。**系が判定してよいか** である。

法令族の行では、要件充足の判定をコードに置いている。しかし「この支出は損金算入できる」という結論そのものは、要件が書けていても系は出さない。理由は精度ではない。弁護士法 72 条・税理士法 52 条・社労士法 27 条が、他人の個別事案への当てはめを業として行うことを資格者に限定しているからである（[houki-hub](https://github.com/shuji-bonji/houki-hub) は、この線で family の射程を切っている）。

同じことは法令以外でも起きる。医療行為の適否、与信の可否、懲戒の量定 ── 基準が文書化されていても、判定主体が法令・契約・社内規程で限定されている領域がある。

```mermaid
flowchart TD
    Q["この項目の判定を、どこに置くか"]
    Q --> A{"系が判定してよいか<br/>(法令・契約・責任)"}
    A -->|いいえ| OUT["系の外<br/>資格者・権限者へ渡す"]
    A -->|はい| B{"基準を事前に書けるか"}
    B -->|はい| CODE["判定層 = コード<br/>ルール表 + firedRules"]
    B -->|いいえ| ESC["human_review_required"]
    CODE --> NAR["解説層 = LLM"]
    ESC --> NAR
    OUT --> NAR
...
> [!NOTE]
> 「系の外」と `human_review_required` は、下流の見た目が似ていて中身が違う。前者は **基準が整備されても系に戻ってこない**。後者は、欠けていたファクトが揃えば次回はコードで判定できる。両者を同じ値に畳むと、前者がいつまでも「あと少しで自動化できる項目」として扱われる。
...
6. **ルールが参照する入力のうち、LLM が生成・改変できるものを列挙する。** 1 番は自然言語の混入を止めるが、LLM が作った構造化データが観測層に混ざる経路までは塞がない。「分類ラベルが `public` なら自動承認」というルールで、そのラベルを提案側の LLM が付けているなら、判定しているのは実質 LLM である。列挙したものは「判定はコードにある」という主張の抜け穴として扱う（[HEXIS I0 の抜け道](https://github.com/shuji-bonji/HEXIS/blob/main/docs/04-invariants.md)）。
...
判定をコードに出しても、解説層には固有の故障が残る。書記官は判決主文を書き写すが、**写し間違える**。

| 故障 | 検査（コードで行う） |
| --- | --- |
| 項目の取り違え・転記漏れ | 出力の項目数と `firedRules` の件数を突合する |
| ルール ID の付け違い・捏造 | 出力中の ID が `firedRules` に実在するか照合する |
| 逸脱の言い換え・緩和 | verdict 語彙を四値の固定リストと照合する |
| 出力書式の崩れ | スキーマ検証を通す |

いずれも LLM に「注意させる」のではなく、解説層の出力を受け取った側がコードで検査する。プロンプトでの指示は対策に数えない。
...
- [ ] 判定を **系に置いてよい** かを、基準が書けるかとは別に確認したか
...
- [ ] ルールが参照する入力のうち、**LLM が生成・改変できるもの** を列挙したか
- [ ] 解説層のプロンプトに「判定を再評価してよい」と読める余地が無いか
- [ ] 解説層の出力を `firedRules` と **突合** しているか
...
- [understanding-llm / 解説層に残る故障](https://shuji-bonji.github.io/understanding-llm-through-claude-code/ja/appendix/narrative-layer-failures) — 判定をコードに出したあと、解説層に残る 4 つの故障と 8 問題との対応
