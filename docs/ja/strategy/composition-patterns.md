...
> [!NOTE]
> ここで扱うのは **異なるドメインの MCP を横に組む** 場合である。同じドメインを役割で縦に割って「族」として設計する場合（PDF family、DTIR family 等）は、分割軸・相互非依存の原則・閉ループを扱う [MCP Family](./mcp-family) を参照。
...
- **MCP呼び出しの並列化**: 独立したMCP呼び出しは並列実行して[コンテキストウィンドウ](../glossary#context-window)を効率的に使う
