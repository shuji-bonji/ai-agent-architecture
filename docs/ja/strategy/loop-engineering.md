...
「内側ループは ReAct 以来すでに解決済みで、誰も `while` 文では競争していない。新しいのは、そのループを囲う**外側のループ**だ」という整理は、2026 年に Boris Cherny（Claude Code の作者）や Karpathy の発言を起点に広まった。本ページはこの議論を、本サイトの 5 層モデル・[ハーネス](../glossary#harness) 4 責務・LLM の構造的制約に接地させる。
...
エージェントがツールを呼ばなくなったのは、**ターンが終わった**だけで、**仕事が終わった**わけではない。テストがまだ落ちているのに「進捗したので完了です」と宣言する —— これは [Sycophancy](../glossary#structural-problems)（甘い自己判断）の典型だ。

- 上限ガード（最大ラウンド / [トークン](../glossary#token)・時間・コストの上限）は **MUST**。
...
ターンを重ねるほど、古いツール出力・行き止まり・陳腐化した推論が[コンテキスト](../glossary#context)に溜まり、出力品質が落ちる（**[Context Rot](../glossary#structural-problems)**）。汚れた文脈がさらに悪い判断を生み、ノイズを増やす悪循環は「doom loop」と呼ばれる。
