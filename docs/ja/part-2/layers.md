...
| 四段の梯子（Prompt / Context / Harness / Loop Engineering）を五層と同じだと思う | 軸が違う。梯子は設計の対象がどこまで外へ広がったか、五層は何をどこへ置くか |
...
四段の梯子と五層は、別々の軸である。梯子は、設計の対象がモデルに送る言葉からどこまで外へ広がったかを示す。五層は、その広がった先で何をどこへ置くかを示す。梯子を一段上っても層は減らないし、五層を決めただけでは外側ループが自動で回るわけではない。梯子の全体は [序章 0.2](../preface)、外側ループの自動化は [Loop Engineering](../strategy/loop-engineering) に書く。
