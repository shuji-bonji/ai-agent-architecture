...
> - **サブエージェント** = 「専門家」を別[コンテキスト](../glossary#context)で起動（中間結果はメインに流入しない）
...
| 質問 | 根拠 |
| --- | --- |
| **Q1: 中間呼出が多い?** | 探索的なツール呼出 (例: コードベース横断検索) は数百〜数千[トークン](../glossary#token)の中間結果を生む。親に流入すると [Context Rot](../glossary#structural-problems) のリスクが高まる → サブエージェント |
...
| **Q4: ロール / 人格?** | 「セキュリティ監査官として」「翻訳家として」など、ロール固定が品質に直結する場合 → サブエージェント ([システムプロンプト](../glossary#system-prompt)が分離される) |
