...
各フェーズを「次へ進める状態」にあるかを判定するためのチェックリスト。**前のフェーズのゲートを通過してから次へ進む** ことを推奨する。Concepts セクションの[出口チェックリスト](../concepts/#concepts-実装への出口チェックリスト)が「設計思想の理解」を確認するのに対し、こちらは「各フェーズの実装準備」を確認する。

```mermaid
flowchart LR
    G0["Concepts<br/>出口ゲート"]
    G1["G1: 戦略・企画<br/>ゲート"]
    G2["G2: 要件定義<br/>ゲート"]
    G3["G3: 設計<br/>ゲート"]
    G4["G4: 実装<br/>ゲート"]
...
- [ ] **MUST/SHOULD/MAY 分類** — 規範強度ラダー（[Concepts → 規範強度ラダー](../concepts/#規範強度ラダー-shall-should-may)）に従って要件を分類したか？
...
- [ ] **三層分離の遵守** — Agent / Skills / MCP の責務境界が設計に反映されているか？
