...
参照先としての信頼性を判断する最初のステップは、情報の発信元がどの程度権威あるものかを確認することです。

### 1.1 発信元の確認

以下の項目により、発信元の信頼性を評価します。
...
参照先が「ブレない」ためには、時間が経過しても変更されない、または適切に版管理されていることが重要です。

### 2.1 版管理の確認

版の管理状態を確認することで、参照の安定性を判断できます。
...
参照先として有効であるためには、ドキュメントが適切に構造化され、セクションや用語が一意に識別可能である必要があります。

### 3.1 参照可能性の確認

構造化の度合いを確認することで、参照の曖昧性を排除できます。
...
参照先として信頼できるためには、読者が原文を確認でき、記述の根拠を独立して検証できることが重要です。

### 4.1 原典アクセスの確認

原典への直接アクセスが可能であることを確認します。
...
AI駆動開発の文脈では、参照先がMCP（Model Context Protocol）経由で自動的にアクセス可能であることが有用です。

### 5.1 MCP化可能性の評価

参照先をMCP化する際のコストと実現可能性を評価します。
...
上記の5つの特性をまとめて評価し、参照先としての総合的な適格性を判定します。

### 評価テンプレート

以下のテンプレートを使用して、参照先を体系的に評価できます。
...
| 非公式翻訳                 | 原文との乖離リスク       |
| AI生成コンテンツ（未検証） | [ハルシネーション](./glossary#structural-problems)の可能性 |

## 8. 参照先の更新管理

採用した参照先は定期的に確認し、新版の発表や廃止、セキュリティ問題などの変化に対応する必要があります。

### 8.1 定期確認項目

参照先の有効性を保つための定期確認項目を以下に示します。
...
参照先に変更が発生した場合の対応手順を以下のフロー図で示します。
