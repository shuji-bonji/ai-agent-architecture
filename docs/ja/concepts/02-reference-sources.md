...
本ドキュメントは、[Vision（01-vision）](./01-vision)が示す「なぜAI駆動開発にブレない参照先が必要か」という問いに対し、**その世界観を成立させる唯一の物理的根拠**を提供する。このアーキテクチャはゼロから発明されたものではなく、RFC・W3C・法令といった人類が積み上げてきた標準体系を、AI時代の設計基盤として再構築するものである。
:::
...
| **依存** | [01-vision](./01-vision)（AIの制約の定義） |
...
> **注**: このドキュメントは主に「外部の権威ある情報源」について扱う。チーム内のドメイン知識・ベストプラクティスについては [Skills](../skills/what-is-skills) を参照。MCPとSkillsの使い分けは [vs-mcp.md](../skills/vs-mcp.md) を参照。

## なぜAIは「ブレる」のか

### 1.1 確率的生成の本質

大規模言語モデル（LLM）は、本質的に**確率的なテキスト生成システム**である。以下のフローチャートは、LLMがテキストを生成する基本的なプロセスを示している。
...
| 特性                       | 説明                                                         | AIへの影響                                 |
| -------------------------- | ------------------------------------------------------------ | ------------------------------------------ |
| **統計的パターン学習**     | 学習データ中の共起パターンから「次に来やすい[トークン](../glossary#token)」を予測 | 「正しい」ではなく「もっともらしい」を出力 |
...
**[ハルシネーション](../glossary#structural-problems)の発生メカニズム**
...
AIの出力には**説明責任（Accountability）の主体がない**。以下の図は、従来の情報源とAI出力における責任の所在の違いを示している。
...
検証可能性は、[Vision](./01-vision)で定義した「知識変換の第3軸：検証化（Spec-to-Test）」と直結する。仕様を参照するだけでなく、仕様を検証可能なテストに変換することで、AIの出力に対する品質の閉ループが形成される。
...
参照先は信頼性と拘束力に基づいて4つのレベルに分類される。各レベルの遵守義務は [RFC 2119](https://datatracker.ietf.org/doc/html/rfc2119) の用語に準じる。
...
チーム・プロジェクト内で統一すべきルール。外部API経由のアクセスは不要なため、**Skills による体系化が主な提供手段**となる。
...
| ------------------ | ----------------------- | -------------------------------------- | ---------- | ------------------------------------------------------- |
| **rfcxml-mcp**     | IETF RFC                | 構造取得、要件抽出、チェックリスト生成 | ✅ 運用中   | [GitHub](https://github.com/shuji-bonji/rfcxml-mcp)     |
| **w3c-mcp**        | W3C/WHATWG/IETF Web標準 | WebIDL、CSS、HTML要素                  | ✅ 運用中   | [GitHub](https://github.com/shuji-bonji/w3c-mcp)        |
| **hourei-mcp**     | 日本法令（e-Gov）       | 法令検索、条文取得                     | ✅ 運用中   | [GitHub](https://github.com/ryoooo/e-gov-law-mcp)       |
| **pdf-spec-mcp**   | PDF仕様（ISO 32000）    | 構造取得、要件抽出、バージョン比較     | ✅ 運用中   | [GitHub](https://github.com/shuji-bonji/pdf-spec-mcp)   |
...
> 本ドキュメントが示す「ブレない参照先」は、[Vision](./01-vision)が掲げる「検証可能な設計思想」の物理的基盤である。Visionで定義した責任シフトモデル（設計時・実行時・構造的制約）と二層検証構造（ガードレール・評価パイプライン）は、ここで体系化した参照先の特性と階層構造の上に成立する。
