...
| ---------------------- | ------------------------------ | ------------------------------------------------- |
| **Observe（観察）**    | コンテキスト収集               | MCP層 + Agent層（✅ 部分的にカバー）              |
| **Orient（方向付け）** | 判断基準の適用                 | Skills層が暗黙的に寄与（⚠️ **明示的には未定義**） |
| **Decide（決定）**     | 優先順位付け、トレードオフ解決 | Agent層が暗黙的に実行（⚠️ **明示的には未定義**）  |
| **Act（実行）**        | ツール経由の実行               | MCP層 + Skills層（✅ カバー済み）                 |

**Orient** と **Decide** — 判断基準と意思決定原則が存在する場所 — は、Skills層やAgent層が暗黙的に担っているが、現在のアーキテクチャにはこれらを**明示的に定義する専用の場所**がない。ここに**ドクトリン層**が位置する。
...
#### OODAマッピングの比較：三層モデル → 四層モデル

ドクトリン層の追加により、三層モデルで「暗黙的」だったOrient/Decideが**明示的にカバー**される。

| OODAフェーズ           | 三層モデル（従来）                            | 四層モデル（ドクトリン追加後）                                   |
| ---------------------- | --------------------------------------------- | ---------------------------------------------------------------- |
| **Observe（観察）**    | MCP層 + Agent層（✅ 部分的にカバー）          | MCP層 + Agent層（✅ 変更なし）                                   |
| **Orient（方向付け）** | Skills層が暗黙的に寄与（⚠️ 明示的には未定義） | **ドクトリン層**が判断基準を明示的に供給 + Skills層（✅ カバー） |
| **Decide（決定）**     | Agent層が暗黙的に実行（⚠️ 明示的には未定義）  | **ドクトリン層**が優先順位を明示的に定義 + Agent層（✅ カバー）  |
| **Act（実行）**        | MCP層 + Skills層（✅ カバー済み）             | MCP層 + Skills層（✅ 変更なし）                                  |

ドクトリン層は Observe や Act を直接担うのではなく、**Orient（何を基準に判断するか）と Decide（何を優先するか）を明示的に定義する**ことで、既存層の暗黙的な判断を「原則に基づく意思決定」へ昇格させる。
