...
::: tip ドクトリン層
これら三層は**AIが何を知り、何ができるか**を定義する。**AIが何を基準に判断し、決定するか**は、[ドクトリン層](./07-doctrine-and-intent)が担う。ドクトリン層は共有された目的・制約・判断基準を通じて、三層すべてを統治する。
:::
...
ここまで説明してきたレイヤー構造の全体像を、最後にまとめて示す。なお、[ドクトリン層](./07-doctrine-and-intent)（制約・目的・判断基準）が、以下に示すすべての層を暗黙的に統治している。
