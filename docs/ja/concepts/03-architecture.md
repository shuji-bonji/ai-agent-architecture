...
[Vision（01-vision）](./01-vision)が「なぜ」を、[ブレない参照先（02-reference-sources）](./02-reference-sources)が「何を」を定義するのに対し、本ドキュメントは **「どう構成するか」** を定義する。思想を壊さずに構造化し、哲学的基盤から実行可能なシステム設計への橋渡しを担う。
:::
...
| | |
| --- | --- |
| **この章で固定するもの** | Agent / Skills / MCP の三層分離と各層の責務境界 |
| **扱わないこと** | 各パターンの選択基準（→04）、制約への対処（→05）、ドクトリン層の詳細（→07） |
| **依存** | [01-vision](./01-vision)（設計思想）、[02-reference-sources](./02-reference-sources)（参照先の体系） |
| **誤用ポイント** | 三層を物理的な配置と混同すること。三層は**責務の分離**であり、デプロイ構成ではない |
:::
...
block-beta
    ...
    block:AGENT_LAYER:1
        columns 1
        AGENT["エージェント層\nタスク理解 / オーケストレーションの決定 / 結果の統合"]
    end

    block:SKILL_LAYER:1
        columns 1
        SKILL["スキル層\nドメイン知識 / ベストプラクティスとガイドライン / 意思決定基準"]
    end

    block:MCP_LAYER:1
        columns 1
        MCP["MCP レイヤー\n外部 API アクセス / ツール実行 / データ取得"]
    end

    EXTERNAL["外部サービス（DeepL、RFC Editor、GitHub など）"]:1
...
| **MCP**    | 外部接続                       | ツール定義         | deepl-mcp, rfcxml-mcp        |
...
flowchart TB
    ...
    subgraph PROTOCOL_LAYER["プロトコル層"]
        MCP_CLIENT["MCP Client"]
        A2A_CLIENT["A2A Client"]
    end

    subgraph EXTERNAL_LAYER["外部サービス層"]
...
[参照先の4層モデル](./02-reference-sources#_3-1-4層モデル)において、レベル3（組織規約）とレベル4（ベストプラクティス）は Skills による体系化が主な提供手段となる。Skillの主な特徴を以下に整理する。
...
> この例は、Visionで定義した[二層検証構造](./01-vision#責任のシフトモデル)の具体的な実装パターンである。用語集の遵守が**ガードレール**（不可侵制約）、xCOMETスコアが**評価パイプライン**（確率的品質ゲート）に対応する。
...
三層モデル（Agent / Skills / MCP）は、**エージェントが何を知り、何ができ、何を基準に判断するか**を定義する。しかし、エージェントが**過去の対話や結果をどう記憶し、どう活用するか**——すなわち Memory——は、このモデルのスコープ外である。

::: tip なぜ三層に含めないのか
Memory は本質的に**動的**であり、対話の進行とともに変化する。一方、Skills は静的なドメイン知識、MCP は明示的なプロトコルインターフェースであり、どちらも**宣言的に定義可能**な要素である。Memory はこれらと性質が異なり、同列に置くとモデルの明快さが失われる。
...
ここまで説明してきたレイヤー構造の全体像を、最後にまとめて示す。[ドクトリン層](./07-doctrine-and-intent)（制約・目的・判断基準）が、すべての層を統治する。情報フローとして、ドクトリンの制約は上位から下降し、リソースの事実は下位から上昇し、エージェントの意思決定は中央で行われる。
...
block-beta
    ...
    USER["ユーザー"]:1
    HOST["ホスト（Claude Code / Claude.ai）"]:1
    SKILLS["Skills（静的知識・ガイドライン）"]:1
    MAIN["メインエージェント ← オーケストレーション"]:1
    SUBAGENT["カスタムサブエージェント ← 役割・専門性の定義"]:1
    PROTOCOL["MCP Client / A2A Client ← プロトコル処理"]:1
    EXTERNAL["MCP Servers / 外部A2Aエージェント ← ツール・外部サービス"]:1
