...
> [!NOTE] 本サイトのスコープ
> 本サイトで「AIエージェント」と言うとき、それは **基盤モデル (主に LLM) を推論コアとするエージェント** を指す。強化学習エージェントやシンボリック AI を含む AI 全体の中での位置づけは [FAQ: 「AIエージェント」は LLM だけを指すのか](../faq/scope-of-ai-agent) を参照。
...
AIは学習データから確率的に出力を生成するが、以下を保証できない。各制約に対する解決手段は同質ではない点に注意する。

| AIの限界   | 説明                                                 | 解決手段                                                            |
| ---------- | ---------------------------------------------------- | ------------------------------------------------------------------- |
| **正確性** | [Hallucination](../glossary#structural-problems)問題 - 事実と異なる情報を生成する可能性 | **MCP**（権威ある情報源への接続）                                   |
| **最新性** | 学習データのカットオフ以降の情報を持たない           | **MCP**（動的取得）                                                 |
| **権威性** | 仕様の正式な解釈を保証できない                       | **MCP**（一次ソースの直接引用）                                     |
| **責任性** | 法的・倫理的判断の根拠を示せない                     | **Skills + Doctrine + 人間のレビュー**（接続だけでは解決しない）    |

上3つは「信頼できるソースに接続する」ことで構造的に補完できる。**一方、責任性は接続だけでは解決しない。** 法は houki シリーズの MCP で参照できても、倫理的判断や組織固有の価値判断は、Skills として体系化したドメイン知識と、[Doctrine 層](./07-doctrine-and-intent) による判断基準の明示、そして最終的な人間のレビューを組み合わせて初めて担保される。

::: tip 接続で解ける課題 / 解けない課題
本ドキュメントの設計思想は「接続で解ける課題」と「Skills + Doctrine + 人間で解く課題」を分離することにある。前者は MCP の役割、後者は本サイトが繰り返し強調する **人間に残る責任** に対応する。
:::
...
一方、今日のソフトウェア開発において、AIが生成するコードはフレームワークやライブラリといった抽象化レイヤーに依存しており、その根底には人類が積み上げてきた**標準規格・仕様**がある。**しかしAIはその標準規格を直接参照できておらず（後述の図で破線）**、抽象化レイヤー越しの推測に頼っている。
...
AIが生成したコードはこの標準規格の上に成り立っているにもかかわらず、**AIの学習データには抽象化レイヤー（ライブラリのコード例）が支配的に含まれ、規格の一次ソースへの直接参照経路が失われている**（破線）。これが現在の問題である。

**AIが正確に機能するには、コードが依拠する標準規格・仕様をAI自身も直接参照できる状態にする**必要がある。これが「ブレない参照先」が必要な理由である。
...
| [ハルシネーション](../glossary#structural-problems)     | 検証可能な根拠の提供           |
| 文脈による解釈のブレ | 一貫した判断基準               |
| 最新情報の欠如       | 最新仕様の取得                 |
...
- 高コスト（コンサル・専門家・出版コスト）
- 一方通行（読者から専門家へのフィードバックが届かない）
- 言語障壁（翻訳者ボトルネック・タイムラグ・訳の品質依存）
- 知識の劣化（伝言ゲーム的に正確性が落ちる）
...
**高額なコンサルや専門家、あるいは抽象化レイヤー（ライブラリ・フレームワーク）に過度に依存しなくても、一次ソースに基づいた開発ができるようになる**。
...
flowchart LR
    ...
    AI -->|"生成"| Output
    Test -->|"品質ゲート<br/>(合否判定)"| Output
    Output -.->|"不合格なら再生成<br/>(③ の駆動ループ)"| AI
...
③の「検証化」は、①②と異なり品質の**閉ループ**を形成する（図中の破線フィードバック）。仕様を渡すだけではAIの出力は検証不能だが、仕様をテストに変換し、合否判定を経て不合格時に再生成へフィードバックすることで初めて「駆動」が成立する。
...
::: tip テストファースト思考はAI駆動開発でも有効
従来のTDD（Red-Green-Refactor）がそのまま適用できるわけではないが、**テストファースト思考**——すなわち「仕様を検証可能な形に変換してから実装する」——はAI駆動開発においても本質的に有効である。確率的な出力を扱う以上、**検証可能なゲート**を実装より先に決めておくことが、AI駆動開発の品質設計の出発点となる。
:::

### 相互補完の構造

「人間とAIが得意な領域」は、しばしば**似た言葉でも対称ではない**。たとえば「品質判断」と「品質チェック」は別物で、前者は人間が得意、後者はAIが網羅性で勝る。以下の対照表で、両者の非対称性を具体的に整理する。

| タスク | 人間 | AI | 補足 |
| --- | :---: | :---: | --- |
| 品質**判断**（基準を決める） | ✅ | — | 「何を良いとするか」を価値判断する |
| 品質**チェック**（基準に照らして検出） | △ | ✅ | 人間は見落としやすい / AIは網羅性で勝る |
| ステークホルダーとの**信頼関係構築** | ✅ | — | 表情・文脈・暗黙の合意を読む |
| **情報の網羅的収集**（仕様・規格） | △ | ✅ | 疲労せず、抜けが少ない |
| 倫理的・**価値観に基づく判断** | ✅ | — | Skills と Doctrine で補助はできるが代替不可 |
| **定型作業の高速反復**（リファクタ・命名統一） | — | ✅ | スループットで勝る |
| **創造的な仮説生成** | ✅ | △ | AI は組み合わせは得意だが、起点の問いは人間 |
| **多言語翻訳・要約** | △ | ✅ | xCOMET 等の評価指標で品質も担保可能 |

この非対称性を踏まえると、次の Mermaid 図の「人間の役割」と「AIの役割」がなぜそう配置されているかが理解しやすくなる。
...
::: warning 読者との契約
本ドキュメントは「AIをどう信頼可能な環境に置くか」の設計思想を示すものであり、特定の品質水準や安全性を約束するものではない。ただし、検証不能な状態を放置するのではなく、**仕様を検証可能なテストに変換し、ガードレールと評価パイプラインの二層で品質を担保する**アプローチを採る。**最終的な判断と責任は、常に人間の側にある。**
:::
