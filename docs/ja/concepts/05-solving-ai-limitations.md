...
| **依存** | [01-vision](./01-vision)（4制約の定義）、[04-ai-design-patterns](./04-ai-design-patterns)（パターンの整理） |
| **誤用ポイント** | 「MCP を導入すれば制約が解消する」という過度な期待。制度的制約（権威性・責任性）は技術だけでは解決できない |
:::

## このドキュメントについて

[01-vision](./01-vision) と [02-reference-sources](./02-reference-sources) では、AIの4つの根本的制約（正確性・最新性・権威性・責任性）を**問題として定義**した。[04-ai-design-patterns](./04-ai-design-patterns) では、RAGやMCPなどの**設計パターン**を整理した。

本ドキュメントでは、それらの制約に対して**現時点で実際に使える解決アプローチ**を整理する。完璧な解決策は存在しないが、各制約の性質を理解し、適切なアプローチを組み合わせることで、実用的なレベルまでリスクを低減できる。

### このドキュメントが解決しないこと

本ドキュメントは以下を**約束しない**。誤解を避けるために明記する。

| 本ドキュメントが示すこと      | 本ドキュメントが保証しないこと       | 代わりに提供するもの                 |
| ----------------------------- | ------------------------------------ | ------------------------------------ |
| 各制約の解決可能性の境界      | すべてのAI制約が技術で解消できること | 制約ごとの現実的な対処戦略           |
| MCP/RAG等による軽減アプローチ | [ハルシネーション](../glossary#structural-problems)の完全排除           | 検証ワークフロー + 不確実性の明示    |
...
flowchart TB
    subgraph SOLVABLE["技術で解決可能"]
        CURRENCY["最新性<br>知識カットオフ"]
    end

    subgraph MITIGABLE["技術で軽減可能"]
        ACCURACY["正確性<br>ハルシネーション"]
        AUTHORITY["権威性<br>解釈の正式性"]
    end

    subgraph SYSTEMIC["制度設計が必要"]
        ACCOUNTABILITY["責任性<br>法的・倫理的根拠"]
    end
...
[03-architecture](./03-architecture) の3層モデル（Agent / Skills / MCP）が、各制約にどう寄与するかを整理する。

```mermaid
flowchart LR
    subgraph Architecture["Architecture 3層"]
        Agent["Agent Layer<br/>推論・検証ループ"]
        Skills["Skills Layer<br/>判断基準・ベストプラクティス"]
        MCP_L["MCP Layer<br/>外部権威ソースへのアクセス"]
    end

    subgraph Constraints["4つの制約"]
        C1["最新性 ◎"]
        C2["正確性 △"]
        C3["権威性 △"]
        C4["責任性 ✗"]
...
AIの制約を軽減する上で中核となるのが、**確率的推論**（LLMの生成）と**決定論的検証**（ツールによる照合）を分離する設計である。

```mermaid
flowchart TB
    subgraph Probabilistic["確率的推論（LLM）"]
        G["回答生成<br/>もっともらしい出力"]
    end

    subgraph Deterministic["決定論的検証（MCP/Tools）"]
        V1["validate_statement()<br/>原典との照合"]
        V2["出典の特定<br/>セクション番号まで"]
        V3["確信度の判定<br/>Confirmed / Estimated / Uncertain"]
    end
...
::: tip 限定された自律性
このプロジェクトのエージェントは「何でも自律的に判断する」ことを目指さない。エージェントの自律性は**検証可能な範囲に限定**される。検証できない判断については人間にエスカレーションする設計とする（[責任シフトモデル](./01-vision#責任シフトモデル)参照）。
:::
...
LLMの知識は学習データの時点で固定される（詳細: [02-reference-sources 1.2.2](./02-reference-sources)）。しかし、この制約に対しては**外部情報源への接続**という明確な解決手段がある。

### 1.2 解決アプローチ

```mermaid
flowchart TB
    PROBLEM["最新性の制約<br>知識カットオフ"]

    PROBLEM --> WEB["Web検索<br>（リアルタイム情報取得）"]
    PROBLEM --> MCP_RT["MCP経由のリアルタイムアクセス<br>（権威ある情報源への直接接続）"]
    PROBLEM --> RAG_UP["RAGインデックスの定期更新<br>（組織内知識の最新化）"]
...
ハルシネーション（事実と異なる情報の生成）は、LLMが「正しい」のではなく「もっともらしい」出力を確率的に生成するという**本質的な性質**に起因する（詳細: [02-reference-sources 1.2.1](./02-reference-sources)）。
...
```mermaid
flowchart TB
    PROBLEM["正確性の制約<br>ハルシネーション"]

    PROBLEM --> INJECT["外部知識の注入<br>（MCP / RAG）"]
    PROBLEM --> VERIFY["生成結果の検証<br>（多段階チェック）"]
    PROBLEM --> DECLARE["不確実性の明示<br>（AIが自身の限界を伝える）"]
    PROBLEM --> HUMAN["人間によるレビュー<br>（最終確認プロセス）"]

    INJECT --> EVAL["△ 軽減可能<br>完全排除は不可"]
    VERIFY --> EVAL
    DECLARE --> EVAL
    HUMAN --> EVAL
...
AIの出力は「一つの解釈」であり、**公式な見解ではない**（詳細: [02-reference-sources 1.2.3](./02-reference-sources)）。RFC、法令、標準仕様の「正式な解釈」を提供できるのは、その策定主体（IETF、立法機関等）のみである。
...
```mermaid
flowchart TB
    PROBLEM["権威性の制約<br>解釈の正式性"]

    PROBLEM --> DIRECT["原典への直接アクセス<br>（MCPで一次情報源を取得）"]
    PROBLEM --> CITE["出典の明示<br>（セクション番号まで特定）"]
    PROBLEM --> WORKFLOW["人間レビューのワークフロー<br>（判断は人間が行う）"]
    PROBLEM --> MULTI["多角的な検証<br>（複数のMCPで横断確認）"]

    DIRECT --> EVAL["△ リスク管理として対応<br>完全解決は不可"]
    CITE --> EVAL
    WORKFLOW --> EVAL
    MULTI --> EVAL
...
> 出力テンプレートの詳細は [02-reference-sources 第4章](./02-reference-sources) を参照。
...
sequenceDiagram
    ...
    AI-->>U: 技術・法律の両面からの分析<br>⚠️ 最終判断は専門家の確認を推奨
```
...
AIの出力には**説明責任（Accountability）の主体がない**（詳細: [02-reference-sources 1.2.4](./02-reference-sources)）。これは技術的な問題というよりも、法的・倫理的な問題である。
...
```mermaid
flowchart TB
    PROBLEM["責任性の制約<br>法的・倫理的根拠の欠如"]

    PROBLEM --> AUDIT["監査証跡の確保<br>（Audit Trail）"]
    PROBLEM --> TRACE["トレーサビリティ<br>（判断の過程を記録）"]
    PROBLEM --> INTEGRITY["データ完全性の保証<br>（電子署名・タイムスタンプ）"]
    PROBLEM --> GOVERNANCE["ガバナンス体制の構築<br>（人間の責任体制を明確化）"]

    AUDIT --> EVAL["✗ 技術は基盤を提供<br>制度設計が不可欠"]
    TRACE --> EVAL
    INTEGRITY --> EVAL
    GOVERNANCE --> EVAL
...
flowchart TB
    subgraph TECH["技術的アプローチ"]
        direction LRE
        MCP_APPROACH["MCP<br>構造化アクセス"]
        RAG_APPROACH["RAG<br>類似度検索"]
        WEBSEARCH["Web検索<br>リアルタイム情報"]
        VALIDATION["検証ワークフロー<br>多段階チェック"]
    end

    subgraph ORG["組織的アプローチ"]
        direction LR
        REVIEW["人間レビュー<br>最終承認"]
        GOVERNANCE["ガバナンス<br>責任体制"]
    end

    subgraph FUTURE_TECH["将来の技術"]
        direction LR
        SIGNATURE["電子署名<br>データ完全性"]
        TIMESTAMP["タイムスタンプ<br>存在証明"]
    end
...
AIの制約を「すべて技術で解決する」ことはできない。**完全に解消するわけではない**ことを前提に、各制約の性質に応じた戦略を選択する。
...
- [01-vision](./01-vision) — AIの限界の定義と「ブレない参照先」の必要性（WHY）
- [02-reference-sources](./02-reference-sources) — 5つの特性による参照先の体系（WHAT）
- [03-architecture](./03-architecture) — MCP/Skills/Agentの構成論（HOW）
- [04-ai-design-patterns](./04-ai-design-patterns) — RAG/MCP等の設計パターン比較（WHICH）
- [mcp/what-is-mcp](../mcp/what-is-mcp) — MCPの詳細
- [07-doctrine-and-intent](./07-doctrine-and-intent) — ドクトリン層：制約・目的・判断基準
