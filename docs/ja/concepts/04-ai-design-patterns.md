...
前章までの抽象的なアーキテクチャを、**具体的な設計判断に落とし込む**ためのパターン集である。各パターンは「採用すべきもの」ではなく、**条件が整えば成立するが、前提が崩れると機能しなくなる構造**として記述している。
:::
...
| **依存** | [03-architecture](./03-architecture)（三層モデルの構造定義） |
| **誤用ポイント** | パターンを「銀の弾丸」として扱うこと。各パターンには Failure Risk（崩壊条件）がある |
:::

## このドキュメントについて

生成AI（LLM）を実用的なシステムに組み込むためには、モデル単体の能力だけでは不十分である。AIの知識には限界があり（[01-vision](./01-vision) 参照）、それを補うためにさまざまな設計パターンが生まれてきた。
...
flowchart LR
    ...
    subgraph Patterns["Design Patterns（本ページ）"]
        direction LR
        P1["知識注入パターン<br/>RAG / MCP / GraphRAG"]
        P2["推論強化パターン<br/>Prompt Engineering / CoT"]
        P3["自律行動パターン<br/>Agentic AI"]
        P4["モデル強化パターン<br/>Fine-tuning"]
    end

    Architecture -->|"構造が制約を与える"| Patterns
    Patterns -->|"パターンが構造を実現する"| Architecture
...
flowchart LR
    subgraph MCP
        Q2[ユーザーの質問] --> A2[AIが判断]
        A2 --> T[ツール呼び出し<br>例: get_requirements]
        T --> S2[MCPサーバー]
        S2 --> E2[外部サービス<br>RFC / DeepL等]
        E2 --> R2[構造化された結果]
        R2 --> G2[回答生成]
    end
```
...
| **コスト**   | 高い（学習データ準備 + 計算資源）              |
| **強み**     | モデルに深く知識を埋め込める                   |
| **弱み**     | 更新が困難、[ハルシネーション](../glossary#structural-problems)の完全排除は不可能 |
...
::: tip 「崩壊リスク」の読み方
各パターンは「銀の弾丸」ではない。崩壊リスク列は、そのパターンが**機能しなくなる主な条件**を示す。設計時にはこのリスクに対するフォールバック戦略を検討すること。
:::
...
graph TB
    subgraph 実際のシステム
        PE[Prompt Engineering<br>ベースとなる指示]
        RAG_COMP[RAG<br>社内文書の検索]
        MCP_COMP[MCP<br>標準仕様の参照]
        AGENT[Agentic AI<br>オーケストレーション]
    end
...
| **プロンプト肥大化**   | Prompt Engineeringで全てを制御しようとする | [コンテキストウィンドウ](../glossary#context-window)を圧迫し、本来の知識注入の余地がなくなる           |

> アンチパターンの回避には、各パターンの**前提条件**と**崩壊リスク**（2.4節の比較表参照）を理解した上で選択することが重要である。
...
| **標準化**       | なし（RFC・W3C仕様は存在しない）             | [modelcontextprotocol.io](https://modelcontextprotocol.io) | [agentskills.io](https://agentskills.io) |
...
つまり、RAGは「検索して → [コンテキスト](../glossary#context)に注入して → 生成する」という**概念レベルの合意**があるだけで、具体的な実装方法は実装者に委ねられている。

### 3.5.2 RAGはエージェント内部の処理パイプライン

RAGの処理は、ユーザーから見るとエージェントの**内部処理**として実行される。

```mermaid
flowchart TB
    subgraph USER_LAYER["ユーザーが制御する領域"]
        SKILL["Skills<br>SKILL.md で知識・判断基準を定義"]
        MCP_CONFIG["MCP<br>外部データソースへの接続を定義"]
        CLAUDE_MD["CLAUDE.md<br>プロジェクト全体のルールを定義"]
    end

    subgraph AGENT_LAYER["エージェント内部（ユーザーが直接触れない）"]
        RAG["RAGパイプライン<br>検索 → ランキング → 注入"]
        CTX["コンテキストウィンドウ管理"]
        TOOL["ツール選択・実行判断"]
    end

    SKILL -->|読み込み| AGENT_LAYER
    MCP_CONFIG -->|接続設定| AGENT_LAYER
    CLAUDE_MD -->|ルール適用| AGENT_LAYER
...
```mermaid
flowchart LR
    subgraph DO["利用者がやるべきこと"]
        S1["Skills を書く<br>判断基準・ベストプラクティスを<br>SKILL.md として定義"]
        S2["MCP を設定する<br>必要な外部データソースへの<br>接続を構成"]
        S3["CLAUDE.md を書く<br>プロジェクト固有の<br>ルール・文脈を記述"]
    end

    subgraph RESULT["結果"]
        R["エージェントがRAG的に<br>これらの知識を活用して<br>高品質な出力を生成"]
    end
...
> **まとめ**: RAGそのものを構築・制御する必要はない。Skills・MCP・CLAUDE.md という**標準化されたインターフェース**を通じて、エージェントに「何を知っているべきか」を伝えることが利用者の役割である。
...
flowchart TB
    subgraph RAG_APPROACH["RAG：テキストの類似度で検索"]
        ...
        R2 --> R3[ベクトル化]
        R3 --> R4[類似度検索]
        R4 --> R5["断片的な結果<br>(文脈喪失リスク)"]
    end

    subgraph MCP_APPROACH["MCP：ドメインの構造で検索"]
        direction TB
        M1[構造化データ] --> M2[ドメイン理解]
        M2 --> M3["構造化クエリ<br>get_requirements()"]
        M3 --> M4["正確な結果<br>+ メタデータ"]
    end
...
flowchart TB
    ...
    Q3 -->|Yes| RAG[RAGを使用]
    Q3 -->|No| PE[Prompt Engineering<br>で対応]
...
sequenceDiagram
    participant U as ユーザー
    participant AI as AI
    participant RAG as RAG<br>(社内Wiki)
    participant MCP as rfcxml-mcp

    U->>AI: WebSocket実装について<br>社内の設計方針と標準仕様を確認して
    ...
    AI-->>U: 社内方針: 〇〇（出典: 社内Wiki）<br>標準要件: △△（出典: RFC 6455 §5.2）<br>⚠️ 社内方針とRFC要件の差異: ...
```
...
graph TB
    subgraph MCPの3つの価値
        V1["① 正確性<br>構造を理解したアクセス"]
        V2["② 検証可能性<br>出典の明示"]
        V3["③ 民主化<br>標準プロトコルで配布"]
    end
...
- [01-vision](./01-vision) — AIの限界と「ブレない参照先」の必要性（WHY）
...
- [skills/vs-mcp](../skills/vs-mcp) — MCPとSkillsの使い分け
